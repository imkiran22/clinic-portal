...
// Combobox-style multi-select. Chips live inline with the search input
// inside a single bordered container so the control sits at the same
// height as a regular input. Clicking any result adds it to the chip
// list; the input self-clears so the next pick is a keystroke away.

export default defineComponent({
  name: 'MultiPatientPicker',
  props: {
    modelValue: {
      type: Array as PropType<Patient[]>,
      default: () => [],
    },
    placeholder: { type: String, default: 'Add patient(s)…' },
  },
  ...
  setup(props, { emit }) {
    ...
    const { data, isFetching } = usePatients(debounced, pageRef)
    ...
    const add = (p: Patient) => {
      ...
      search.value = ''
      // Keep focus so the user can type the next patient immediately.
    ...
    const onBackspace = () => {
      if (search.value === '' && props.modelValue.length > 0) {
        // Backspace into an empty input pops the last chip — same affordance
        // every modern tag input supports.
      ...
      <div ref={containerRef} class="relative">
        ...
        {open.value && debounced.value && (
          <div class="absolute z-30 left-0 right-0 mt-1 max-h-72 overflow-auto rounded-md border border-border bg-popover shadow-lg">
            ...
            {!isFetching.value && matches.value.length === 0 && (
              <div class="px-3 py-2 text-sm text-muted-foreground">
                No more patients match "{debounced.value}".
              </div>
            )}
            {matches.value.map((p) => (
              <button
